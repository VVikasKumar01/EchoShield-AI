...
class DefenseEnhancedLoss(nn.Module):
    """Defense-Enhanced Composite Loss Function.

    Combines:
      1. Multi-Resolution Spectral Loss (speech quality, phase and magnitude reconstruction)
      2. Noise classification Cross-Entropy (supervising the noise characterizer)
      3. Targeted Impulsive Noise Residual Penalty (suppresses gunshot/artillery shock leakage)
      4. Mask loss (ERB envelope accuracy)
    """

    def __init__(
        self,
        df_state: Optional[DF] = None,
        factor_spec: float = 1.0,
        factor_mask: float = 1.0,
        factor_cls: float = 0.1,
        factor_impulse: float = 2.0,
        n_ffts: Iterable[int] = (256, 512, 1024),
        sr: int = 48000,
    ):
        super().__init__()
        ...
        self.factor_impulse = factor_impulse

        if df_state is None:
            df_state = DF(sr=sr, fft_size=960, hop_size=480, nb_bands=32)

        self.spec_loss = MultiResSpecLoss(n_ffts=n_ffts)
        self.mask_loss = MaskLoss(df_state)
    ...
    def forward(
        ...
        clean_spec: Tensor,
        enh_spec: Tensor,
        noisy_spec: Optional[Tensor] = None,
        clean_audio: Optional[Tensor] = None,
        enh_audio: Optional[Tensor] = None,
        mask_pred: Optional[Tensor] = None,
        mask_target: Optional[Tensor] = None,
        noise_logits: Optional[Tensor] = None,
        noise_labels: Optional[Tensor] = None,
        impulse_mask: Optional[Tensor] = None,
    ) -> Tuple[Tensor, dict]:
        """Compute composite loss.

        Args:
            clean_spec (Tensor): Ground truth clean speech STFT [B, 1, T, F, 2].
            enh_spec (Tensor): Model enhanced speech STFT [B, 1, T, F, 2].
            clean_audio (Tensor, optional): Clean audio in time domain.
            enh_audio (Tensor, optional): Enhanced audio in time domain.
            mask_pred (Tensor, optional): Predicted ERB mask [B, 1, T, E].
            mask_target (Tensor, optional): Target ERB mask [B, 1, T, E].
            noise_logits (Tensor, optional): Predicted noise logits [B, 3].
            noise_labels (Tensor, optional): Ground truth noise class labels [B].
            impulse_mask (Tensor, optional): Ground truth or detected impulse indicator [B, T, 1].

        Returns:
            loss (Tensor): Total composite scalar loss.
            metrics (dict): Dictionary of individual loss components.
        """
        metrics = {}
        loss = torch.tensor(0.0, device=clean_spec.device, dtype=clean_spec.dtype)

        # 1. Multi-Resolution Spectral Loss
        if self.factor_spec > 0:
            if clean_audio is not None and enh_audio is not None:
                l_spec = self.spec_loss(clean_audio, enh_audio)
            else:
                # Direct spectral MSE if time-domain waveforms are not passed
                l_spec = F.mse_loss(enh_spec, clean_spec)
            loss = loss + self.factor_spec * l_spec
            metrics["loss_spec"] = l_spec.detach().item()

        # 2. Mask Loss
        if self.factor_mask > 0 and mask_pred is not None:
            if noisy_spec is not None and clean_spec is not None:
                l_mask = self.mask_loss(mask_pred, clean_spec, noisy_spec)
            elif mask_target is not None:
                l_mask = F.mse_loss(mask_pred, mask_target)
            else:
                l_mask = torch.tensor(0.0, device=clean_spec.device)
            loss = loss + self.factor_mask * l_mask
            metrics["loss_mask"] = l_mask.detach().item()

        # 3. Noise Classification Loss
        if self.factor_cls > 0 and noise_logits is not None and noise_labels is not None:
            l_cls = self.cls_loss(noise_logits, noise_labels)
            loss = loss + self.factor_cls * l_cls
            metrics["loss_cls"] = l_cls.detach().item()

        # 4. Impulsive Shock Suppression Penalty
        if self.factor_impulse > 0 and impulse_mask is not None and impulse_mask.sum() > 0:
            # Mask frames where shockwaves occur
            # impulse_mask: [B, T, 1] -> broadcast to [B, 1, T, F, 2]
            mask_5d = impulse_mask.unsqueeze(1).unsqueeze(-1)
            error_spec = (enh_spec - clean_spec) * mask_5d
            l_imp = torch.sum(error_spec ** 2) / (mask_5d.sum() * enh_spec.shape[-2] * 2 + 1e-8)
            loss = loss + self.factor_impulse * l_imp
            metrics["loss_impulse"] = l_imp.detach().item()
