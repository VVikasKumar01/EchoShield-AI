...
class InstantaneousImpulseLimiter(nn.Module):
    """Detects and soft-clamps sudden broadband impulsive acoustic shocks (gunshots, artillery).

    Impulses exhibit extreme energy spikes across all frequencies in short time spans (< 10ms).
    Without pre-recurrent protection, such shockwaves push GRU hidden states into deep saturation,
    causing speech swallowing and trailing muting artifacts for several frames afterwards.
    """

    def __init__(
        self,
        crest_thresh_db: float = 12.0,
        energy_thresh_rel: float = 8.0,
        max_atten_db: float = 24.0,
    ):
        """Initialize the impulse limiter.

        Args:
            crest_thresh_db (float): Peak-to-average subband energy threshold in dB.
            energy_thresh_rel (float): Relative frame energy increase threshold (ratio to moving average).
            max_atten_db (float): Maximum attenuation applied to detected impulse frames.
        """
        super().__init__()
        self.crest_thresh_db = crest_thresh_db
        self.energy_thresh_rel = energy_thresh_rel
        self.max_atten_lin = 10.0 ** (-max_atten_db / 20.0)

    def forward(
        self, spec: Tensor, feat_erb: Optional[Tensor] = None
    ) -> Tuple[Tensor, Optional[Tensor], Tensor]:
        """Detect impulse shocks and apply soft limiting.

        Args:
            spec (Tensor): Complex spectrogram of shape [B, C, T, F, 2].
            feat_erb (Tensor, optional): ERB features of shape [B, C, T, E].

        Returns:
            spec_limited (Tensor): Soft-attenuated spectrogram [B, C, T, F, 2].
            feat_erb_limited (Tensor, optional): Soft-attenuated ERB features.
            impulse_mask (Tensor): Indicator of impulse events of shape [B, T, 1].
        """
        # Calculate instantaneous frame energy: [B, C, T, F]
        power = spec[..., 0] ** 2 + spec[..., 1] ** 2
        frame_energy = power.mean(dim=(-2, -1), keepdim=True) + 1e-10  # [B, C, 1, 1] or per time
        if spec.dim() == 5:
            # per-frame energy along T: mean over C and F
            frame_energy_t = power.mean(dim=(1, 3), keepdim=True)  # [B, 1, T, 1]
            peak_energy_t = power.max(dim=3, keepdim=True)[0].mean(dim=1, keepdim=True)  # [B, 1, T, 1]
        else:
            frame_energy_t = power.mean(dim=-1, keepdim=True)
            peak_energy_t = power.max(dim=-1, keepdim=True)[0]

        # Subband Crest factor in dB: 10 * log10(Peak / Mean)
        crest_factor_db = 10.0 * torch.log10((peak_energy_t + 1e-10) / (frame_energy_t + 1e-10))

        # Rolling / baseline frame energy across time:
        rolling_mean = frame_energy_t.mean(dim=-2, keepdim=True) + 1e-10
        energy_ratio = frame_energy_t / rolling_mean

        # Impulse criterion: both substantial energy surge and broadband distribution
        is_impulse = (energy_ratio > self.energy_thresh_rel) & (crest_factor_db < self.crest_thresh_db)
        # Squeeze to [B, T, 1]
        impulse_mask = is_impulse.squeeze(1).float()  # [B, T, 1]

        # Calculate soft gain: 1.0 when not impulse, max_atten_lin when impulse
        gain = torch.where(
            is_impulse,
            torch.full_like(frame_energy_t, self.max_atten_lin),
            torch.ones_like(frame_energy_t),
        )

        spec_limited = spec * gain.unsqueeze(-1)
        feat_erb_limited = feat_erb * gain if feat_erb is not None else None

        return spec_limited, feat_erb_limited, impulse_mask
